...
//--------------------------------------------------------------------------------------------------------------//
// 	Modulo anterior de toma de decodificación de comnado AT														//
//--------------------------------------------------------------------------------------------------------------//
	 /*
//3/------------------Módulo reseteado ------------------//
	a->_n_fcomp=strlen("Ai-Thinker Technology Co.,Ltd.");
	 if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"Ai-Thinker Technology Co.,Ltd.",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Transmision ok
	 {
	  	AT_decode=4;
	  	a->_debug_ready++;
	 }
...
//2/------------------WIFI DESCONECTADA------------------//
	...
	{
//3/------------------CLIENTE DESCONECTADO DE SERVIDOR ------------------//
	a->_n_fcomp=strlen(",CLOSED\r\n");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,",CLOSED\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//if (strcmp(a->_uartRCVD,"CLOSED\r\n")==0)//Cerrado TCP desde la APP
	{
		AT_decode=25;
	...
	{
//1/------------------TCP CAIDO DESPUES DE CONECTADO------------------//
   // En el envío contínuo el CLOSED se concatena con el SEND OK y si //
   // primero ve el SEND OKse queda en el estado 705 por error al     //
   //  enviar														  //
   //-----------------------------------------------------------------//
...
//3/------------------CONFIRMO ENVIO DE INFO OK ------------------//
	 a->_n_fcomp=strlen("\r\nSEND OK\r\n");
 	 if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"\r\nSEND OK\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Transmision ok
 		{
 		AT_decode=at_tcp_enviado_ok;
 		a->_debug_SEND_OK++;
 		}
 		...
 		{
 			a->_n_fcomp=strlen("\nSEND OK\r");
 			if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"\nSEND OK\r",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)
 			{
...
//1/------------------INTENTAR ENVIAR DATOS EN TCP CAIDO------------------//
	a->_n_fcomp=strlen("link is not valid");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"link is not valid",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//if (strcmp(a->_uartRCVD,"CLOSED\r\n")==0)//Cerrado TCP desde la APP
	{
		AT_decode=at_tcp_snd_err;
	...
	{
//4/------------------CONFIRMO CIERRE DE  CONEXION------------------//
	a->_n_fcomp=strlen("AT+CWQAP\r\r\n\r\nOK\r\n");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"AT+CWQAP\r\r\n\r\nOK\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)
	 {
		 AT_decode=13;
	 }
	 else
...
//7/------------------TCP CAIDO AL INTENTAR CONECTAR------------------//
		 a->_n_fcomp=strlen("\r\nERROR\r\nCLOSED\r\n");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"\r\nERROR\r\nCLOSED\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Intentando conectar con servidor TCP caido
...
//8/------------------TCP CONECTADO------------------//
	 ...
	 {
//15/------------------DESCONECTAR TCP CUANDOYA EST� DESCONECTADO------------------//
	a->_n_fcomp=strlen("AT+CWMODE=1\r\r\n\r\nOK\r\n");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"AT+CWMODE=1\r\r\n\r\nOK\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Error desconectar TCP ya desconectado
	{
		 AT_decode=at_tcp_enviado_ok;
	}
	else
	{
//15/------------------CONECTAR TCP YA CONECTADO------------------//
	a->_n_fcomp=strlen("\r\nALREADY CONNECTED\r\n");
	if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"\r\nALREADY CONNECTED\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Error desconectar TCP ya desconectado
	{
		 AT_decode=at_tcp_alrdy_cnntd_err;
...
//9/------------------ERROR------------------//
     ...
     {
//10/------------------TCP CERRADO DESDE EL MODULO OK------------------//
		 a->_n_fcomp=strlen("AT+CIPCLOSE\r\r\nCLOSED\r\n\r\nOK\r\n");
	 if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"AT+CIPCLOSE\r\r\nCLOSED\r\n\r\nOK\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Desonectado desde el modulo
	 {
		 AT_decode=9;
	 }
	 else
...
//13/------------------DESCONECTAR TCP CUANDOYA EST� DESCONECTADO------------------//
		 a->_n_fcomp=strlen("AT+CIPCLOSE\r\r\n\r\nERROR\r\n");
	 if (FT_String_ND(a->_uartRCVD,&a->_n_orig,"AT+CIPCLOSE\r\r\nCLOSED\r\n\r\nOK\r\n",&a->_n_fcomp,a->_uartRCVD_tok,&a->_n_tok,&chr_pos_fnc,&a->_id_conn,FIND)==1)//Error desconectar TCP ya desconectado
	 {
		 //AT_decode=12;
...
//------------------INSTANCIA DEDEBUG A ELIMINAR --------------------------
 ...
 //Eval�o todo lo recibido desde el puerto serie
		...
		//la cantidad de datos a enviar, es posible recorrer todo el vector con un while y contar hasta el ultimo dato (que lo detecte
										...
										//STRNACT_ADV(a->_uart2snd,strlen("AT+CIPSEND="),caracteres,strlen(caracteres));
...
/*							if(b->_estado==at_tcp_srvr_err)//		//Reinicio involuntario de módulo
						...
						case TCP_SRVR_OK://Conectar Servidor TCP local OK
						{
							NOP(b);
								...
								if(((b->_estado==at_tcp_alrdy_cnntd_err)||(b->_estado==at_tcp_conectado)) && ModBUS_Check(b->_data2SND,b->_n_D2SND))//if((b->_estado==at_tcp_alrdy_cnntd_err)||(b->_estado==at_tcp_conectado))
								{
									EnviarDatosSRVR(b);
								...
								if(b->_send_data==1) // SOLO HTTP     if((b->_send_data==1)&& ModBUS_Check(b->_data2SND,b->_n_D2SND))
								{
									EnviarDatosSRVR(b);
									b->_estado_conexion=TCP_SRVR_TX_EN_CURSO;
									b->_estado=0;
						...
						case TCP_SRVR_TX_EN_CURSO:			//Enviar datos desde Servidor TCP
						{
							if((b->_estado!=900)&&(b->_estado!=at_error)&&
							   ...
							   (b->_estado!=22)&&(b->_estado!=25))//210630 Se agrega 11 y 12 //Se agrega 22 y 25, conexion cerrada y errore de serverif((b->_estado!=900)&&(b->_estado!=2)&&(b->_estado!=at_wifi_disconnect)&&(b->_estado!=at_tcp_desconectado)&&(b->_estado!=at_tcp_snd_err)&&(b->_estado!=at_tcp_enviado_error)&&(b->_estado!=at_tcp_enviado_ok))	//Si estoy conectando, no vuelvo a conectar.
							{
								EnviarDatosSRVR(b);
									if(b->_enviaruart==1)
										{
						...
						case TCP_SRVR_TX_OK://Enviar datos a Servidor TCP OK
						{
							NOP(b);
								...
								if(((b->_new_data_rcv==1)||(b->_send_data==1))) // OJO SOLO CON HTTP if(((b->_new_data_rcv==1)||(b->_send_data==1))&& ModBUS_Check(b->_data2SND,b->_n_D2SND))
								{
									/*if(b->_data2SND[10]==100)
									{
										b->_data2SND[10]=0x00;
									}
									b->_data2SND[10]++;*/
